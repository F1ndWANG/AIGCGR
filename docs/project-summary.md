...
- SQLite 运行时推荐历史、用户反馈、饮食记录和生活状态记录。
- SQLite 长期用户偏好画像，包括默认地点、默认预算、口味、忌口、过敏、健康目标和旅行偏好。
- SQLite 计划列表，推荐项可从卡片直接加入计划，并支持完成或取消。
- 前端可切换用户 ID，饮食、偏好、反馈和计划按 `user_id` 隔离。
...
- Active Plans 侧栏，展示已加入计划的推荐项，并可直接标记完成或取消。
- Provider 能力状态展示。
...
- 计划列表当前支持状态管理，但还没有接入日历提醒、通知或自动导航。
